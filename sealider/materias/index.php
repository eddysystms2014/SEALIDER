		...
		<div class="gtco-container">
			<div class="row">
				<div class="col-md-12">
					...
					<h3>Control de Materias</h3>
					<form class="form-horizontal" action="../controller/controllerMat.php">
					<div class="form-group">
						<label class="control-label col-sm-4">INSTRUCTOR:</label>
						<div class="col-sm-8">
						<select class="form-control" name="ced_instructor">
						<option value="0">--Seleccionar--</option>
						<?php
						//include_once '../model/Database.php';
						//$pdo =  Database::connect();
						$pdo = new mysqli('localhost', 'root', '', 'sealider');
						$query = $pdo->query("SELECT * FROM instructor");
						while ($valores = mysqli_fetch_array($query)) {
							echo '<option value="'.$valores[CED_INSTRUCTOR].'">'.$valores[NOMBRE_APELLIDO].'</option>';
							}?>
							</select>
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-sm-4">NOMBRE MATERIA:</label>
						<div class="col-sm-8">
							<input type="text" class="form-control" pattern="[a-zA-Z\s]*" name="descripcion" required>
						</div>
					</div>
					<div class="form-group">
	...
	<div class="gtco-container">
		<div class="row">
			<div class="col-md-12">
				...
				<form action="../controller/controllerMat.php">
            	...
            	<tr>
            		<th>ID</th>
                	<th>CED. INSTRUCTOR</th>
               		<th>NOMBRE MATERIA</th>
                	<th>ELIMINAR</th>
                	<th>ACTUALIZAR</th>
            	</tr>
            	<?php
            	include_once '../model/Materias.php';
            	if (isset($_SESSION['listado'])) {
                $listado = unserialize($_SESSION['listado']);
                foreach ($listado as $mat) {
                   	echo "<tr>";
                   	echo "<td>" . $mat->getID_MATERIA() . "</td>";
                   	echo "<td>" . $mat->getCED_INSTRUCTOR() . "</td>";
                   	echo "<td>" . $mat->getDESCRIPCION() . "</td>";
                   	echo "<td class='warning'><a href='delete.php?op=eliminar&id_materia=" . $mat->getID_MATERIA() . "'><img src='../images/eliminarmat.png' style='width:2em;height:2em'></a></td>";
                   	echo "<td class='active'><a href='../controller/controllerMat.php?op=editar&id_materia=".$mat->getID_MATERIA()."'><img src='../images/actualizarmat.png' style='width:2em;height:2em'></a></td>";
                   	echo "</tr>";
               		}
            	} else {
